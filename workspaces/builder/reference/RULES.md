...
1. Save your current state to `memory/session-state.json` (Sunny — see your SKILL.md for the full schema)
...
|-----------|------------|-------------------|
| Sub-agent | Sunny agent only | Other sub-agents, Sunny, the human |
| (removed — no Sunny layer) | David (via Telegram) and your sub-agents | Other Sunny, the human directly |
| Board member | Advisory board channel only | Sunny, sub-agents, the human |
| Master VA | The human + Sunny + advisory board | Sub-agents directly |
...
When delegating a task (Sunny only), every delegation must include:
...
|------|--------|-----------|---------------|
| 1 | Sub-agent self-debugs with Sunny guidance | 3 back-and-forths | Escalate to Tier 2 |
| 2 | Sunny consults internal consultant (Opus) | 3 rounds | Escalate to Tier 3 |
| 3 | Sunny makes a decision | N/A | Retry differently, escalate to Master VA, or mark blocked |

**Total: 6 attempts before a hard decision.** This prevents infinite debug loops while giving enough room for resolution.

**Master VA escalation:** If a Sunny escalates the **same problem 2 or more times**, the Master VA consults the advisory board. This signals the issue is strategic, not operational.
...
On session start, if a sub-agent finds a WIP file, report to Sunny:
```
Found interrupted work on [task]. Last state: [summary]. Resume?
```

Wait for Sunny confirmation before continuing — the task context may have changed.
